...
- “第一次用、第一次来、帮我建立初始画像”：以 `student-quick-assessment` 为主，组合 `learning-dna`；先请用户拿出一份近期材料，快速测评并生成会话内初版 DNA，保存前单独确认。
- “你能做什么、平时怎么跟你说、给我示例”：`system-guide`；介绍能力类别和自然说法，不展示内部 playbook 菜单。若同时明确是第一次使用，先执行上一条首次启动流程。
...
- 需要创建真实提醒、OCR、夜间分析或看板：切换到 `k12-automation`；仅给文本计划不需要切换。夜间分析只经 `references/adapters/night-analysis-v1.md` 公开契约传递，不暴露内部 playbook 路径。
