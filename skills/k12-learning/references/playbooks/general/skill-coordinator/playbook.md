...
description: >
  ...
  并在任务过易或过难时进行学习区校准。
  当 k12-learning 内部决策判定为 COMPOSE，或学生明确说“帮我生成全景月报”“学习系统运转得好吗”
  “把多个学习环节串起来”时激活。普通单任务直接使用专业 playbook，不经过本协调器。
  仅在当前任务需要且用户已同意相关数据使用时，按最小必要字段汇总，不默认跨 playbook 全量拉取或写回。
compatibility: Claude Code / Codex / OpenClaw / ClawHub
references:
  - references/coordinator-operating-manual.md
  - references/trigger-routing-rules.md
...
- 仅当 module 内部决策判定为 `COMPOSE`，或学生明确要求多个学习环节联动、系统健康检查、全景月报、学习链补链时激活。
- “这道题该用什么方法”由 `k12-learning` 主流程判断；本 playbook 不重新路由。
