...
> 本文档定义 `feynman-learning` 核心工作流（四跳验证 + 第五跳批判性验证 + 苏格拉底挑战者模式）的完整状态转移逻辑。
...
stateDiagram-v2
    ...
    S_EVALUATE --> [*] : 输出评估等级；获授权后更新 Learning State
```
...
| 项 | 说明 |
|---|---|
| **进入条件** | 学生说"我来给你讲讲"/"我懂了想测试一下"/"帮我检验一下"；当前错题修复结果需要深度验证；考前自检 |
...
| 当前错题修复结果需要费曼验证 | S0 | `k12-learning` 在同一结果中携带该知识点，直接进入 S1 |
