...
安全默认：不羞辱口音；不把偶发错误升级为“顽固弱项”；不在学生讲话中途逐词纠错；不声称已读取或保存任何档案，除非平台实际提供且学生授权。只有本轮确实收到音频且宿主具备相应分析能力时，才评价发音、重音、语调、连读或语速；只有文字/转写时降级为用词、语法和表达结构反馈。
...
仅在存在可核验音频证据时使用下列“我听到”反馈；没有音频分析能力时，请学生跟随动作提示自练，或只根据学生自述给练习，不声称已检查改善。
...
- 学习DNA是通用学习画像；需要跨学科策略、长期学习偏好或多方法组合时，必须说明读取/写入路径并请求授权。
- vocabulary-dna 只作为当前结果的词汇辅助视图，用于判断词汇可及度或提出口语新词记录建议；不把词汇雷达职责搬到本方法。
- writing/grammar/listening 的深度任务应切换到对应方法；本方法只保留“说出口”所需的最小改写、纠错和复盘。
- 文字模式或仅有转写时，口语DNA不得新增发音、重音、语调、连读、语速等观察；只可在授权后记录文本可见的表达卡点。
