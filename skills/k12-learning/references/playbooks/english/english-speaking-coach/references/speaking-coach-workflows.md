# english-speaking-coach · 执行细则归档

> 本文件承接 `playbook.md` 瘦身前的详细流程。主文件只保留触发、最小闭环、边界和索引；需要完整话术、口语DNA、晨间热身、四级跳、纠音、角色扮演和协作细则时读取本文。
...
description: >
  ...
  凡是涉及英语口头表达、开口练习、发音训练的场景，务必采用此方法。
...
1. **判断触发**：确认本次任务是否属于英语口语表达、发音、晨读或情景对话训练；如果不是，先指出更合适的方法，或仅做普通回答。
2. **收集最小输入**：只追问完成任务必需的信息：话题、学生表达草稿/转写、目标场景和想提升的维度。信息不足时，先给可选模板让学生补充，不臆造长期数据。
3. **执行主流程**：按后续章节推进，优先用追问、验证、例子和小步反馈帮助学生自己完成关键思考；不要把可训练的步骤直接替学生做完。
4. **产出结果**：给出可说出口的表达稿、发音/流利度建议和下一轮口语任务，并明确“现在能做的下一步”。
...
- **无音频分析能力**：只有文字或转写时，改做用词、语法、结构和表达任务；不得评价发音、重音、语调、连读、语速或把这些内容写入口语DNA。
...
Step 5  存入DNA
  ...
  → 学生明确同意后，在同一 Learning State 更新中加入该词；不同意则只保留本轮建议
```
...
```
主结果：口语任务 → 开口练习 → 最多三处反馈 → 再说一次
可选辅助视图：兴趣话题、一个词汇任务、一项语法改正
Learning State：读取兴趣/发音历史或记录好词、弱点前必须获授权
真实提醒：切换到 `k12-automation`，由 Automation 再确认时间、渠道和频率
```
