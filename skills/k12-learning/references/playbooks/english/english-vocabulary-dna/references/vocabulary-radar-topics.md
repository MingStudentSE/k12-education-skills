...
> 本文档为 `english-vocabulary-dna` 的配套参考资源，提供新词汇雷达（模块C：课前7天预热）的话题分类模板、词汇分组策略与四阶段预热内容生成指南。
...
词汇雷达：课前7天的预热（"还没学就开始见"）
  ...
  → 结束后：词汇进入正式学习 → 给出五轮复测建议；不自动创建任务

五轮复测：学完后的巩固（"学了之后不能忘"）
  → 目标：通过主动提取逐步形成更稳定的独立使用与迁移证据
  → 完成固定轮次不等于进入长期记忆；根据最后一次真实表现决定延长、保持或缩短下一次间隔
  → 需要真实定时提醒时，切换到 `k12-automation` 并由 Automation 取得调度授权

衔接流程：
  预热Day 7 → 预热Day 5 → 预热Day 3 → 上课学习 →
  → 默认起始模板：Day 1(24h) → Day 3 → Day 7 → Day 14 → Day 30
  → 每轮按独立答对、提示依赖和迁移表现调整，不机械走完固定日期
```
