...
> 本文件保留完整流程、模板、话术、月报和 Product Module seam 细节。主文件只保留路由骨架；需要执行细则时读取本文件。
...
1. **判断触发**：确认本次任务是否属于英语语法纠错、句子升级或语法模式追踪；如果不是，先指出更合适的方法，或仅做普通回答。
2. **收集最小输入**：只追问完成任务必需的信息：英文句子/作文片段、学生想表达的中文意思、是否要追踪长期错误。信息不足时，先给可选模板让学生补充，不臆造长期数据。
3. **执行主流程**：按后续章节推进，优先用追问、验证、例子和小步反馈帮助学生自己完成关键思考；不要把可训练的步骤直接替学生做完。
4. **产出结果**：给出错误定位、语法解释、改写版本和同类练习，并明确“现在能做的下一步”。
...
```
主结果：语法证据 → 规则追问 → 改正句 → 变式验证
可选辅助视图：
  - 写作材料暴露语法问题：保留作文语境，加入一项语法训练
  - 固定搭配或词义问题：加入词汇任务，不建立方法间消息
  - 定语从句达到专项条件：在同一结果加入专项训练
状态与副作用：
  - 更新语法或词汇 Learning State 前取得明确授权
  - 真实提醒切换到 `k12-automation`，由 Automation 再确认调度授权
```
...
- `references/english-error-dimension-table.md` — 英语错因维度表（五维×子类型分类体系，含跨维度关联规则与英语专项方法维度分配）
