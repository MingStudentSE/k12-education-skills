...
> 本文件保留 `english-writing-coach` 的三维批改、句式升级、写作 DNA、场景脚本、月报与 Product Module seam 模板。主文件只保留触发、路由、边界和验收骨架；执行细则以本文为准。
...
1. **判断触发**：确认本次任务是否属于英语作文构思、改稿、表达升级或考试写作训练；如果不是，先指出更合适的方法，或仅做普通回答。
2. **收集最小输入**：只追问完成任务必需的信息：作文题、草稿、评分要求和学生当前瓶颈。信息不足时，先给可选模板让学生补充，不臆造长期数据。
3. **执行主流程**：按后续章节推进，优先用追问、验证、例子和小步反馈帮助学生自己完成关键思考；不要把可训练的步骤直接替学生做完。
4. **产出结果**：给出结构诊断、句段升级、错误清单和可提交版本，并明确“现在能做的下一步”。
...
```
主结果：审题立意 → 学生初稿 → 三维反馈 → 改稿验证
可选辅助视图：一项语法训练、三至五个词汇升级候选
Learning State：读取语法摘要或记录语法、词汇、写作进化前必须获授权
真实提醒：切换到 `k12-automation`，由 Automation 再确认任务与调度授权
```
