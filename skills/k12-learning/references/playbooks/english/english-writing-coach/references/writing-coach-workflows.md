...
> 本文件归档 `english-writing-coach` 原 SKILL.md 中的三维批改、句式升级、写作DNA、场景脚本、月报与跨 Skill 协作模板。主文件只保留触发、路由、边界和验收骨架；执行细则以本文为准。
...
1. **判断触发**：确认本次任务是否属于英语作文构思、改稿、表达升级或考试写作训练；如果不是，先指出更合适的 SKILL 或仅做普通回答。
