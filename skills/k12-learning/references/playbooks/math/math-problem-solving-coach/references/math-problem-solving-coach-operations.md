...
> 本文件承接原 `playbook.md` 的长流程、模块说明、话术模板与考前处理细则；主文件只保留触发路由和执行骨架。
...
1. **判断触发**：确认本次任务是否属于数学解题指导、策略追问或题目复盘；如果不是，先指出更合适的方法，或仅做普通回答。
2. **收集最小输入**：只追问完成任务必需的信息：题面、学生尝试、已知条件和当前卡点。信息不足时，先给可选模板让学生补充，不臆造长期数据。
3. **执行主流程**：按后续章节推进，优先用追问、验证、例子和小步反馈帮助学生自己完成关键思考；不要把可训练的步骤直接替学生做完。
4. **产出结果**：给出解题入口、关键步骤、思路来源和同类题迁移，并明确“现在能做的下一步”。
...
> **关于“拍照解析”机制：** 本模块核心的“四步拍照法”对错题图片的上传与识别，强依赖于多模态大语言模型（Multimodal LLMs）的视觉解析能力，或宿主平台配置的本地 OCR 扩展，并非纯文本 LLM 原生支持。
...
第四步：给出待确认的记录摘要
分析完成后，在同一结果列出：
题型 + 错误类型 + 根本原因 + 修复任务 + 复测标准
只有用户同意后，才更新 Learning State；不自动建立记录
```
...
三、已授权错因摘要对照（可选辅助视图）
    只有用户同意读取 Learning State 时，使用该知识模块的最小历史错误摘要
    → "你在这个模块历史上出错最多的是[X]，
       今天重点注意这个。"
```
...
```
主结果：读题与尝试 → 卡点追问 → 学生完成关键一步 → 变式验证
可选辅助视图：已授权错因摘要、概念解释或进阶梯度题
Learning State：读取历史弱项或记录本次错因与掌握证据前必须获授权
真实提醒：切换到 `k12-automation`，由 Automation 再确认复习任务与调度授权
```
