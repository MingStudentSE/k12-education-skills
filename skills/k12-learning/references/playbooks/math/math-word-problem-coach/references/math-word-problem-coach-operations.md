...
> 本文件承接原 `playbook.md` 的完整使命说明、数量关系三步法、专项追问库、分类追问、迁移训练、输出模板与协作细则；主文件只保留触发路由和执行骨架。
...
1. **判断触发**：确认本次任务是否属于数学应用题建模、数量关系提取或应用题复盘；如果不是，先指出更合适的方法，或仅做普通回答。
...
```
主结果：情境读图 → 数量关系 → 数学模型 → 求解与情境检验
可选辅助视图：读题错因判断或建模后的单题解题追问
Learning State：记录建模错因或更新应用题能力证据前必须获授权
```
