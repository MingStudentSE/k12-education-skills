...
- 通用错题本 `correction-notebook` 已完成数学错题表面记录，并提供基础四维标签、题目摘要、学生答案、正确答案和历史引用。
...
- 概念本身没懂、需要重新讲概念 → 用 `math-concept-explainer`；本方法只给出错因诊断和联动理由。
...
4. 授权状态：是否允许写入档案、回写 `correction-notebook`、向 `k12-automation` 提交提醒请求或提供月报摘要。

信息不足时先发补充模板，不编造学生水平、历史记录或长期数据。

## 核心流程骨架

1. **判定边界**：确认是数学错因诊断而不是通用错题收纳；必要时切换到 `correction-notebook`。
...
- 不在未授权情况下建立长期档案、向 `k12-automation` 提交提醒请求、写入学习DNA或向周复盘外发完整记录。
- 不重复触发预警：数学同类错误3次由本方法执行突破，通用错题本只做统一入口与调度。
...
- `references/math-error-dna-operations.md`：原主文件外移的档案记录格式、图谱/月报模板、数学焦虑流程和内部组合协议。
