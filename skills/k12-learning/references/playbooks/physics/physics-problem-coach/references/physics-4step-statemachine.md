...
> 本文档定义 `physics-problem-coach` 核心工作流（模块A：四步物理解题法）的完整状态转移逻辑。
...
| **卡壳处理** | 学生无法确定模型 → 回退 S1 重新审视图景，追问“图景里哪个信息你没注意到？”；仍无法确定 → 在同一结果加入物理建模方法 |
...
| **常见错误追问** | "你的公式适用条件满足吗？导体本身和温度等状态是否基本不变？" / "这里用串联分压还是并联分流？" |
...
| **AI动作** | ①未授权时只输出本轮错因与掌握摘要 ②用户同意后在同一 Learning State 记录补充 P/C/F/R/T、根因与物理解题证据 ③需要真实提醒时切换到 `k12-automation`，由 Automation 另行取得调度授权 |
...
| 学生无法确定物理模型 | S2_MODEL | 回退 S1 重新审视图景；仍无法确定 → 在同一结果加入物理建模方法 |
...
根据学生描述的问题类型，选择对应CLAW模板：
  ...
  高中拓展中，学生说"我知道用F=ma但不知道怎么列式" → 模板③建模卡壳类
