...
description: >
  ...
  + 顽固弱项识别 + 物理弱项报告。通用错题本记录表面事实，本方法只记录物理根因，不重复存档或触发。
...
适用：物理错因模式、模型混淆、数学工具缺口、顽固物理弱项、物理焦虑信号分析。不适用时切换到普通回答或更合适方法。
...
5. **授权写入**：只有用户同意后才写档案、同步 learning-dna、向 `k12-automation` 提交提醒请求或月报。
...
| 项目 | 通用记录视图 | 物理深度诊断视图 |
|---|---|---|
| 当前输入 | 题目、答案、日期、来源只收集一次 | 使用同一题面、图景和学生步骤 |
| 当前结果 | 通用分类与表面字段 | P/C/F/R/T 子类型、图景证据、跨维度关联与修复任务 |
| 长期状态 | 未授权时不建立记录 | 获授权后补充到同一条 Learning State 记录 |
| 真实提醒 | `k12-learning` 只给复测建议 | 切换到 `k12-automation`，由 Automation 自行取得调度授权 |
...
- 子类型/证据：
- 次要关联：
- 与通用记录的关系：同一结果附加物理深度诊断 / 仅本轮分析 / 获授权后更新同一记录
...
- 边界不清：先用通用错题本记录表面事实，再由本方法做物理根因扩展。
- 高风险联动：未获授权不得更新长期画像、月报、提醒或在内部组合中共享。
