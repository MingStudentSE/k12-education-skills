...
1. **判断触发**：确认本次任务是否属于物理概念直觉、类比或概念边界澄清；如果不是，先指出更合适的 SKILL 或仅做普通回答。
2. **收集最小输入**：只追问完成任务必需的信息：物理概念、现象/实验、学生误解和相关题目。信息不足时，先给可选模板让学生补充，不臆造长期数据。
3. **执行主流程**：按后续章节推进，优先用追问、验证、例子和小步反馈帮助学生自己完成关键思考；不要把可训练的步骤直接替学生做完。
4. **产出结果**：给出直觉模型、类比、适用边界和验证问题，并明确“现在能做的下一步”。
