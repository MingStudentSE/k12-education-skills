...
1. **判断触发**：确认本次任务是否属于物理错因模式、模型混淆或数学工具缺口分析；如果不是，先指出更合适的 SKILL 或仅做普通回答。
2. **收集最小输入**：只追问完成任务必需的信息：物理错题、图像/受力图/电路图、学生步骤和历史错误。信息不足时，先给可选模板让学生补充，不臆造长期数据。
3. **执行主流程**：按后续章节推进，优先用追问、验证、例子和小步反馈帮助学生自己完成关键思考；不要把可训练的步骤直接替学生做完。
4. **产出结果**：给出错因分类、模型混淆点、工具缺口和专项修复任务，并明确“现在能做的下一步”。
