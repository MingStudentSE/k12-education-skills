...
function readStudent(id) {
  ...
  const fm = existsSync(profilePath) ? frontmatter(readFileSync(profilePath, 'utf8')) : {};
...
function main() {
  ...
  const students = ids.map(readStudent).sort((a, b) =>
...
<div class="wrap">
  ...
  <div class="grid">${students.map(card).join('\n') || '<div class="empty">还没有学生，复制 <code>students/_template</code> 建一个。</div>'}</div>
