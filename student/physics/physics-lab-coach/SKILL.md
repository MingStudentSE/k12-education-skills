...
1. **判断触发**：确认本次任务是否属于物理实验设计、数据处理或实验报告复盘；如果不是，先指出更合适的 SKILL 或仅做普通回答。
2. **收集最小输入**：只追问完成任务必需的信息：实验目的、器材、原始数据、图表和学生结论。信息不足时，先给可选模板让学生补充，不臆造长期数据。
3. **执行主流程**：按后续章节推进，优先用追问、验证、例子和小步反馈帮助学生自己完成关键思考；不要把可训练的步骤直接替学生做完。
4. **产出结果**：给出实验步骤诊断、误差分析、数据处理和报告改进建议，并明确“现在能做的下一步”。
