...
> 设计来源：`pipeline/DESIGN_v2.1_intake_and_scoring.md`（Claude Code · GLM-5.2 · max 起草，已收敛）
> 仓库根：`/Users/links/02-Area/k12-education-skills`
> 约束：严格按本指令构建，不得擅自改变 SKILL 边界/字段/维度；如发现设计矛盾，记录在 `pipeline/CODEX_BUILD_NOTES.md` 并按更保守、更贴 SECURITY_BASELINE 的方向决断。
